...
bwimage_t *EEACreateImageOfSize(unsigned int width, unsigned int height){
	unsigned int i;
  bwimage_t *retval=(bwimage_t*)malloc(sizeof(bwimage_t));
  retval->width=width;
  retval->height=height;
  retval->data=(uint8**)malloc(height*sizeof(uint8*));
  retval->rawdata=(uint8*)malloc((width) * (height) * sizeof(uint8));
  for(i=0; i<retval->height; i++){
      (retval->data)[i]=retval->rawdata + (i*retval->width);
  }
  return retval;
}
